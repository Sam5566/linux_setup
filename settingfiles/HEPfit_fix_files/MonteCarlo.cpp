...
: myInputParser(ModelF, ThObsF), MCEngine(ModPars, Obs, Obs2D, CGO, CGP) {
    ...
    // 基于当前系统的当前日期/时间   Sam added
    time_t now = time(0);
    tm *ltm = localtime(&now);
    char timebuffer[80];
    strftime(timebuffer, sizeof(timebuffer), "%m-%d-%H", ltm);
    std::string Time(timebuffer);
    std::cout<<Time<<std::endl;
    ResultDirName = "result_" + Time + "/";   //*//* Sam added to create the "result" dir.

    if (OutFile_i.compare("") == 0) OutFile = ResultDirName + "MCout" + JobTag + ".root";
    else OutFile = ResultDirName + OutFile_i + JobTag + ".root";
    ObsDirName = ResultDirName + "Observables" + JobTag;
...
void MonteCarlo::Run(const int rank) {
    ...
    try {
        ...
        if (rank == 0) {
            FileStat_t info1;
            FileStat_t info2;  //*//* Sam added
            if (gSystem->GetPathInfo(ResultDirName.c_str(), info2) !=0 || gSystem->GetPathInfo(ObsDirName.c_str(), info1) != 0) {
                if (gSystem->MakeDirectory(ResultDirName.c_str()) == 0 &&  gSystem->MakeDirectory(ObsDirName.c_str()) == 0) std::cout << ObsDirName << " directory has been created." << std::endl;
        ...
        if (rank != 0) {
            ...
            if (RunMinuitOnly) {
                BCLog::OpenLog((ResultDirName + "MinuitMinimizationResults" + JobTag + ".txt").c_str(), BCLog::debug, BCLog::debug);
                RedirectHandle_t * rHandle = NULL;
                gSystem->RedirectOutput(("ResultDirName + MinuitMinimizationResults" + JobTag + ".txt").c_str(), "a", rHandle);
...
#endif
            // open log file
            BCLog::OpenLog((ResultDirName + "log" + JobTag + ".txt").c_str(), BCLog::debug, BCLog::debug);
            ...
            if (PrintAllMarginalized) MCEngine.PrintAllMarginalized((ResultDirName + "MonteCarlo_plots" + JobTag + ".pdf").c_str());
            ...
            if (PrintCorrelationMatrix) {
                MCEngine.PrintCorrelationMatrix((ResultDirName + "CorrelationMatrix" + JobTag + ".pdf").c_str());
                MCEngine.PrintCorrelationMatrix((ResultDirName + "CorrelationMatrix" + JobTag + ".tex").c_str());
                MCEngine.PrintCorrelationMatrixToLaTeX((ResultDirName + "ParameterCorrelations" + JobTag + ".tex").c_str());
            }

            // print comparisons of the prior knowledge to the posterior knowledge
            // for all parameters into a pdf file
            if (PrintKnowledgeUpdatePlots) MCEngine.PrintKnowledgeUpdatePlots((ResultDirName + "ParamUpdate" + JobTag + ".pdf").c_str());

            // draw an overview plot of the parameters into a pdf file
            if (PrintParameterPlot) MCEngine.PrintParameterPlot((ResultDirName + "ParamSummary" + JobTag + ".pdf").c_str());
            ...
            if (WritePreRunData) {
                ...
                outPreRun.open((ResultDirName + "PreRun" + JobTag + ".txt").c_str(), std::ios::out);
            ...
            BCLog::SetPrefix(false);
            BCLog::OpenLog((ResultDirName + "MonteCarlo_results" + JobTag + ".txt").c_str(), BCLog::summary, BCLog::nothing);
            ...
            if (PrintTrianglePlot) {
                BCLog::OpenLog((ResultDirName + "log_CorrelationPlots" + JobTag + ".txt").c_str(), BCLog::debug, BCLog::debug);
                MCEngine.PrintCorrelationPlot(ResultDirName + "CorrelationPlots" + JobTag + ".pdf");
                BCLog::CloseLog();
            }
